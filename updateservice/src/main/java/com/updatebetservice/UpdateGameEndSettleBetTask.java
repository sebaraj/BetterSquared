...
public class UpdateGameEndSettleBetTask implements Job {
    ...
    private void updateGameScoreInDatabase(List<GameScore> games) {
        try {
            ...
            for (GameScore game : games) {
                ...
                // If game exists, update it, if it doesnt, skip it
            ...
            try {
                if (dbConnection != null) {
                    dbConnection.close(); // Close database connection in finally block
                }
            } catch (SQLException e) {
                e.printStackTrace();
            }
    ...
    private void settleBetsWithGameID(int gameId, String winner) {
        try {
            ...
            if (dbConnection != null) {
                try {
                    dbConnection.rollback(); // Rollback in case of exception
            ...
            try {
                // Close all prepared statements and reset auto-commit
